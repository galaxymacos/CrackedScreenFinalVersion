...
public class NavMeshSurfaceInPrefabTests
{
    ...
    {
        PrefabStageAutoSavingUtil.SetPrefabStageAutoSave(SessionState.GetBool(k_AutoSaveKey,
            PrefabStageAutoSavingUtil.GetPrefabStageAutoSave()));
        StageUtility.GoToMainStage();
...
#endif
        Assert.IsTrue(HasNavMeshAtPosition(pos, expectedAreaMask), "Expected NavMesh with area {0} at position {1}.",
            expectedArea, pos);
        Assert.IsFalse(HasNavMeshAtPosition(pos, ~expectedAreaMask),
            "A NavMesh with an area other than {0} exists at position {1}.", expectedArea, pos);
    }
    ...
    public void NavMeshSurfacePrefab_WhenOpenedInPrefabMode_DoesNotActivateItsNavMesh()
    {
        ...
        NavMeshHit hit;
        NavMesh.SamplePosition(Vector3.zero, out hit, 1000000f,
            new NavMeshQueryFilter {areaMask = NavMesh.AllAreas, agentTypeID = 0});
        Assert.That(hit.hit, Is.False,
            "The NavMesh instance of a prefab opened for edit should not be active under any circumstances.");
    }
    ...
    public IEnumerator NavMeshSurfacePrefab_AfterBakingInPrefabMode_DoesNotActivateItsNavMesh()
    {
        ...
        NavMesh.SamplePosition(Vector3.zero, out hit, 1000000f,
            new NavMeshQueryFilter {areaMask = NavMesh.AllAreas, agentTypeID = 0});
        Assert.That(hit.hit, Is.False,
            "The NavMesh instance of a prefab opened for edit should not be active after baking the surface.");
    ...
    public IEnumerator NavMeshSurfacePrefab_WhenRebakedButNotSaved_TheRebakedAssetNoLongerExists()
    {
        ...
        Assert.IsFalse(File.Exists(rebakedAssetPath),
            "NavMeshData file still exists after discarding the changes. ({0})", rebakedAssetPath);
    ...
    public IEnumerator NavMeshSurfacePrefab_AfterModifiedInstanceAppliedBack_TheOldAssetNoLongerExists()
    {
        ...
        Assert.IsTrue(File.Exists(initialInstanceAssetPath), "Prefab's NavMeshData file must exist. ({0})",
    ...
    public IEnumerator NavMeshSurfacePrefab_WhenInstanceRevertsBack_TheInstanceAssetNoLongerExists()
    {
        ...
        Assert.IsTrue(File.Exists(instanceAssetPath), "Instance's NavMeshData file must exist. ({0})",
            instanceAssetPath);

        PrefabUtility.RevertPrefabInstance(instance, InteractionMode.AutomatedAction);

        Assert.IsFalse(File.Exists(instanceAssetPath), "Instance's NavMeshData file still exists after revert. ({0})",
            instanceAssetPath);
    ...
    public static bool HasNavMeshAtPosition(Vector3 pos, int areaMask = NavMesh.AllAreas, int agentTypeId = 0,
        float range = 0.1f)
    {
        NavMeshHit hit;
