...
namespace Anima2D
{
                ...
                if (bone && IsVisible(bone))
                {
                    ...
                    if (eventType == EventType.Repaint)
                    {
                            ...
                            Selection.gameObjects.Contains(bone.gameObject))
                        {
                            ...
                            float outlineSize = HandleUtility.GetHandleSize(bone.transform.position) * 0.015f *
                                                bone.color.a;
                    ...
                    if (Selection.activeTransform != transform)
                    {
                            ...
                            (control.bone && !Selection.transforms.Contains(control.bone.transform)))
                        {
                            ...
                            if (Tools.current == Tool.Move)
                            {
                                ...
                                Vector3 newPosition = Handles.FreeMoveHandle(transform.position, transform.rotation,
                                    size, Vector3.zero, Handles.RectangleHandleCap);
#else
								Vector3 newPosition =
 Handles.FreeMoveHandle(transform.position, transform.rotation, size, Vector3.zero, Handles.RectangleCap);
                            ...
                            else if (Tools.current == Tool.Rotate)
                            {
                                ...
                                Quaternion newRotation = Handles.Disc(transform.rotation, transform.position,
