    ...
    public class SpriteMeshEditorWindow : TextureEditorWindow
    {
        ...
        void HandleAddBindPose()
        {
            if (m_SpriteMeshCache.spriteMeshInstance && DragAndDrop.objectReferences.Length > 0)
            {
                ...
                List<GameObject> dragAndDropGameObjects =
                    DragAndDrop.objectReferences.ToList().ConvertAll(o => o as GameObject);
                List<Bone2D> dragAndDropBones =
                    dragAndDropGameObjects.ConvertAll(go => go ? go.GetComponent<Bone2D>() : null);
        ...
        void SetScrollPositionAndZoomFromSprite()
        {
            ...
            float newZoom = Mathf.Min(this.m_TextureViewRect.width / (float) m_SpriteMeshCache.rect.width,
                                this.m_TextureViewRect.height / (float) m_SpriteMeshCache.rect.height) * 0.9f;
            if (m_Zoom > newZoom)
            {
                m_Zoom = newZoom;
            }
        ...
        bool GetLinearSampled(Texture2D texture)
        {
            ...
            MethodInfo methodInfo = typeof(Editor).Assembly.GetType("UnityEditor.TextureUtil")
                .GetMethod("GetLinearSampled", BindingFlags.Static | BindingFlags.Public | BindingFlags.NonPublic);
        ...
        {
            MethodInfo methodInfo = typeof(EditorGUIUtility).GetMethod("SetRenderTextureNoViewport",
