...
namespace Anima2D
{
    ...
    {
        public delegate void CapFunction(int controlID, Vector3 position, Quaternion rotation, float size,
            EventType eventType);
        ...
        public static Vector2 Slider2D(int id, Vector2 position, CapFunction drawCapFunction, Vector3 planeNormal,
        ...
        static void DrawImageBasedCap(int controlID, Vector3 position, Quaternion rotation, float size, GUIStyle normal,
            GUIStyle active)
        {
            ...
            if (Camera.current &&
                Vector3.Dot(position - Camera.current.transform.position, Camera.current.transform.forward) < 0f)
                return;
        ...
        public static void ApplyWireMaterial()
        {
            ...
            {
                s_ApplyWireMaterialMethodInfo = typeof(HandleUtility).GetMethod("ApplyWireMaterial",
        ...
        public static void DrawAAPolyLine(Color[] colors, Vector3[] points)
        {
            ...
            MethodInfo method = type.GetMethod("DrawAAPolyLine",
                BindingFlags.Static | BindingFlags.Public | BindingFlags.NonPublic, null,
                new[] {typeof(Color[]), typeof(Vector3[])}, null);
