    ...
    public class SpriteMeshUtils
    {
        ...
        public static void UpdateAssets(SpriteMesh spriteMesh, SpriteMeshData spriteMeshData)
        {
            if (spriteMesh && spriteMeshData)
            {
                ...
                Vector2[] uvs = (new List<Vector2>(spriteMeshData.vertices))
                ...
                List<WeightedTriangle> weightedTriangles =
                    new List<WeightedTriangle>(spriteMeshData.indices.Length / 3);
            ...
            out List<Vector2> vertices, out List<IndexedEdge> indexedEdges, out List<int> indices)
        {
            ...
            {
                Vector2[][] paths =
                    GenerateOutline(texture, rect, detail, (byte) (alphaTolerance * 255f), holeDetection);
        ...
        static Vector2[][] GenerateOutline(Texture2D texture, Rect rect, float detail, byte alphaTolerance,
            bool holeDetection)
        {
            Vector2[][] paths = null;
