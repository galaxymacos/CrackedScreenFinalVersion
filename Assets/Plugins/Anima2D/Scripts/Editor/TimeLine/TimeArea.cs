...
namespace Anima2D
{
    ...
    internal class TimeArea : ZoomableArea
    {
        ...
        {
            get { return this.m_HTicks; }
            set { this.m_HTicks = value; }
        }

        public TickHandler vTicks
        {
            get { return this.m_VTicks; }
            set { this.m_VTicks = value; }
        }
        ...
        public TimeArea.TimeRulerDragMode BrowseRuler(Rect position, ref float time, float frameRate, bool pickAnywhere,
            GUIStyle thumbStyle)
        {
        ...
        public TimeArea.TimeRulerDragMode BrowseRuler(Rect position, int id, ref float time, float frameRate,
            bool pickAnywhere, GUIStyle thumbStyle)
        {
            ...
            switch (current.GetTypeForControl(id))
            {
                case EventType.MouseDown:
                    ...
                    {
                        GUIUtility.hotControl = id;
                        float num = TimeArea.SnapTimeToWholeFPS(base.PixelToTime(current.mousePosition.x, position),
                            frameRate);
                ...
                case EventType.MouseDrag:
                    if (GUIUtility.hotControl == id)
                    {
                        float num2 = TimeArea.SnapTimeToWholeFPS(
                            base.PixelToTime(current.mousePosition.x - TimeArea.s_PickOffset, position), frameRate);
                        if (num2 != time)
                        {
                            GUI.changed = true;
                        }
        ...
        public string FormatFrame(int frame, float frameRate)
        {
            ...
            return str + (frame / (int) frameRate).ToString() + ":" +
                   ((float) frame % frameRate).ToString().PadLeft(length, '0');
        }
