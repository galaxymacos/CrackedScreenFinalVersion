...
namespace Anima2D
{
    public class RectHandles
    {
        ...
        {
            Vector3 point = new Vector2(Mathf.Lerp(rect.xMin, rect.xMax, (float) xHandle * 0.5f),
                Mathf.Lerp(rect.yMin, rect.yMax, (float) yHandle * 0.5f));
                    ...
                    if (i != 1 || j != 1)
                    {
                        ...
                        if (GUI.color.a > 0f || GUIUtility.hotControl == controlID)
                        {
                            ...
                            if (EditorGUI.EndChangeCheck())
                            {
                                ...
                                if (!alt)
                                {
                                    scalePivot = GetRectPointInWorld(s_StartRect, s_StartPosition, rotation, 2 - i,
                                        2 - j);
                                ...
                                else
                                {
                                    ...
                                    Vector2 newPivot = new Vector2(
                                        ...
                                        s_CurrentRect.yMin +
                                        (s_CurrentRect.yMax - s_CurrentRect.yMin) * s_StartPivot.y);
        ...
        static Quaternion RotationHandlesGUI(Rect rect, Vector3 pivot, Quaternion rotation)
        {
            ...
            for (int i = 0; i <= 2; i += 2)
            {
                for (int j = 0; j <= 2; j += 2)
                {
                    ...
                    float num = RotationSlider(controlID, rectPointInWorld, eulerAngles.z, pivot,
                        rotation * Vector3.forward, outwardsDir, outwardsDir2, handleSize, null, Vector2.zero);
                    if (EditorGUI.EndChangeCheck())
                    {
                        if (Event.current.shift)
                        {
        ...
        static void DrawPolyLineWithOffset(Color lineColor, Vector2 screenOffset, params Vector3[] points)
        {
            ...
            {
                s_TempVectors[i] =
                    (Vector3) HandlesExtra.GUIToWorld(HandleUtility.WorldToGUIPoint(points[i]) + screenOffset);
            }
        ...
        static float RotationSlider(int controlID, Vector3 cornerPos, float rotation, Vector3 pivot, Vector3 handleDir,
            Vector3 outwardsDir1, Vector3 outwardsDir2, float handleSize, HandlesExtra.CapFunction drawFunc,
            Vector2 snap)
        {
            EventType eventType = Event.current.GetTypeForControl(controlID);
