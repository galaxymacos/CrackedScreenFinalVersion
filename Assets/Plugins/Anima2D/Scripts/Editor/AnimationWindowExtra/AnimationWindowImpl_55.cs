...
namespace Anima2D
{
    ...
    {
        protected Type m_CurveBindingUtilityType =
            typeof(EditorWindow).Assembly.GetType("UnityEditorInternal.CurveBindingUtility");
        ...
        public override void InitializeReflection()
        {
            ...
            m_GetCurrentValueMethod = m_CurveBindingUtilityType.GetMethod("GetCurrentValue",
                BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance | BindingFlags.Static, null,
                l_GetCurrentValueTypes, null);
        ...
        public override void CreateDefaultCurve(EditorCurveBinding binding)
        {
            ...
            object curve = Activator.CreateInstance(m_AnimationWindowCurveType,
                new object[] {activeAnimationClip, binding, editorCurveValueType});

            object currentValue = GetCurrentValue(rootGameObject, binding);

            if (activeAnimationClip.length == 0f)
            {
                AddKeyframeToCurve(curve, currentValue, editorCurveValueType,
                    AnimationKeyTime(0f, activeAnimationClip.frameRate));
            }
            else
            {
                AddKeyframeToCurve(curve, currentValue, editorCurveValueType,
                    AnimationKeyTime(0f, activeAnimationClip.frameRate));
                AddKeyframeToCurve(curve, currentValue, editorCurveValueType,
                    AnimationKeyTime(activeAnimationClip.length, activeAnimationClip.frameRate));
            }
        ...
        public override void AddKey(EditorCurveBinding binding, float time)
        {
            ...
            if (curve != null)
            {
                ...
                AddKeyframeToCurve(curve, currentValue, editorCurveValueType,
                    AnimationKeyTime(time, activeAnimationClip.frameRate));
