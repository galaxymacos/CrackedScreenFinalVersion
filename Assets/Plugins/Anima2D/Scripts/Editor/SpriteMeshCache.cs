...
namespace Anima2D
{
    public class SpriteMeshCache : SerializedCache, IVertexManipulable, IRectManipulable
    {
        ...
        [SerializeField] SpriteMeshEditorWindow.Mode m_Mode;
        ...
        [SerializeField] List<Vector2> m_TexVertices = new List<Vector2>();
        ...
        {
            get { return edges.ConvertAll(e => new IndexedEdge(nodes.IndexOf(e.node1), nodes.IndexOf(e.node2))); }
        }
        ...
        [SerializeField] VertexSelection m_Selection = new VertexSelection();
        ...
        {
            get { return nodes.Where((n) => { return selection.IsSelected(n.index); }).ToList(); }
        }
        ...
        public bool isBound
        {
            get { return bindPoses.Count > 0f; }
        }

        public bool isDirty { get; set; }

        [SerializeField] int mSelectedHoleIndex = -1;
        ...
        public Hole selectedHole
        {
            ...
            set { mSelectedHoleIndex = holes.IndexOf(value); }
        }

        public Bone2D selectedBone;

        [SerializeField] int m_SelectedBindPose = -1;
        ...
        public BindInfo selectedBindPose
        {
            ...
            set { m_SelectedBindPose = bindPoses.IndexOf(value); }
        }

        [SerializeField] BlendShape m_SelectedBlendShape;

        public BlendShape selectedBlendshape
        {
            get { return m_SelectedBlendShape; }
            set
            {
        ...
        [SerializeField] RectManipulatorParams m_RectManipulatorParams;

        public RectManipulatorParams rectManipulatorParams
        {
            get { return m_RectManipulatorParams; }
            set { m_RectManipulatorParams = value; }
        }

        [SerializeField] bool m_DirtyVertices = true;

        [SerializeField] List<Vector2> m_CurrentTexVertices = new List<Vector2>();
            ...
            string undoName)
        {
            ...
            Rect clampedRect = MathUtils.ClampRect(MathUtils.OrderMinMax(l_rect),
            ...
            SpriteMeshUtils.InitFromOutline(texture, clampedRect, detail, alphaTolerance, holeDetection,
        ...
        public void CalculateAutomaticWeights(List<Node> targetNodes)
        {
            ...
            {
                Vector2 tip =
                    SpriteMeshUtils.VertexToTexCoord(spriteMesh, pivotPoint, bindInfo.position, pixelsPerUnit);
                Vector2 tail =
                    SpriteMeshUtils.VertexToTexCoord(spriteMesh, pivotPoint, bindInfo.endPoint, pixelsPerUnit);
            ...
            UnityEngine.BoneWeight[] boneWeights = BbwPlugin.CalculateBbw(m_TexVertices.ToArray(),
                indexedEdges.ToArray(), controlPoints.ToArray(), controlPointEdges.ToArray(), pins.ToArray());
        ...
        public void SmoothWeights(List<Node> targetNodes)
        {
            ...
            for (int i = 0; i < indices.Count / 3; ++i)
            {
                for (int j = 0; j < 3; ++j)
                {
                    ...
                    {
                        smoothedWeights[indices[i * 3 + j], k] +=
                            weights[indices[i * 3 + j1], k] + weights[indices[i * 3 + j2], k];
                    }
