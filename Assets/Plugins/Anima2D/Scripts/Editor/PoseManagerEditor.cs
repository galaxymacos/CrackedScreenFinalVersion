...
namespace Anima2D
{
    ...
    public class PoseManagerEditor : Editor
    {
        ...
        void CreateNewPose()
        {
            ...
            Pose newPose = ScriptableObjectUtility.CreateAssetWithSavePanel<Pose>("Create a pose asset", "pose.asset",
                "asset", "Create a new pose");

            mList.serializedProperty.arraySize += 1;

            SerializedProperty newElement =
                mList.serializedProperty.GetArrayElementAtIndex(mList.serializedProperty.arraySize - 1);
