...
namespace Anima2D
{
    public static class MathUtils
    {
        ...
        {
            return new Vector2(Mathf.Clamp(position.x, frame.xMin, frame.xMax),
                Mathf.Clamp(position.y, frame.yMin, frame.yMax));
        }
        ...
        public static float RoundBasedOnMinimumDifference(float valueToRound, float minDifference)
        {
            ...
            return (float) Math.Round((double) valueToRound,
                MathUtils.GetNumberOfDecimalsForMinimumDifference(minDifference), MidpointRounding.AwayFromZero);
        }
